 ...
 *
 * implicit means that these parameters aren't explicitly listed in the
 * function signature, but are silently passed to the function and accessible
 * within the function
 */

/**
 * - arguments is collection of all parameters passed to a function, useful
 *   as it allow us to access all function parameter.
 * - 'arguments' object have property name 'length' indicates the exact number
 *   of argument. value can be obtained by using array indexing notation.
 */
...
/**
 * we can use Rest parameter instead of arguments parameter. rest parameter
 * is a real array so we can use array methods which gives use
 * advantage over arguments.
 */
...
/**
 * arguments object is an alias for the function parameters, if we change
 * the arguments object, the change is also reflected in the matching
 * function parameter.
 *
 * same holds true in the other direction. If we change a parameter, the
 * change can be observed in both the parameter and the arguments object:
 */